...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode==SIGN_IN_CODE) {
            if(resultCode==RESULT_OK) {
                Snackbar.make(activity_main, "Auth", Snackbar.LENGTH_LONG).show();
                displayAllMessages();
            }
            else {
                Snackbar.make(activity_main, "Not auth", Snackbar.LENGTH_LONG).show();
                finish();
            }
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(FirebaseAuth.getInstance().getCurrentUser()==null)
           // registerForActivityResult(AuthUI.getInstance().createSignInIntentBuilder().build(), SIGN_IN_CODE);
